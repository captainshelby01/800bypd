...
<!-- Wix Template 5441 Whimsical Hero Banner Section -->
<div class="relative bg-gradient-to-br from-[#FFF4E5] via-[#FFF9F0] to-[#F3E8FF] border-2 border-[#FDE68A] rounded-[2.5rem] p-8 md:p-16 mb-16 overflow-hidden shadow-sm">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div class="lg:col-span-7 z-10">
            <span class="bg-[#FBBF24] text-[#2D1B4E] font-extrabold text-xs px-4 py-1.5 rounded-full uppercase tracking-wider mb-5 inline-flex items-center gap-1.5 shadow-sm">
                <i class="fa-solid fa-sparkles text-purple-900"></i> Children's Bookstore by WrittenbyPD
            </span>
            <h1 class="font-whimsical text-4xl md:text-6xl font-bold text-[#2D1B4E] leading-[1.1] mb-5 tracking-tight">
            ...
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('catalog.index') }}" class="bg-[#2D1B4E] hover:bg-purple-900 text-white font-extrabold px-8 py-4 rounded-full shadow-lg transition transform hover:-translate-y-0.5 flex items-center gap-2 text-xs uppercase tracking-wider">
                    Shop The Collection <i class="fa-solid fa-arrow-right text-amber-400"></i>
                </a>
                <button onclick="openSpotifyModal()" class="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold px-6 py-4 rounded-full shadow-lg transition flex items-center gap-2 text-xs uppercase tracking-wider">
                    <i class="fa-brands fa-spotify text-base"></i> Listen on Spotify
                </button>
            </div>
        </div>
        ...
        <div class="lg:col-span-5 flex justify-center z-10">
            <div class="relative w-full max-w-sm bg-white p-6 rounded-[2.5rem] shadow-xl border-4 border-[#FBBF24] transform rotate-1 hover:rotate-0 transition duration-300">
                <div class="h-64 bg-purple-50 rounded-2xl flex flex-col items-center justify-center text-center p-6 border border-purple-100 mb-4">
                    <i class="fa-solid fa-book-open-reader text-7xl text-purple-600 mb-3 animate-bounce"></i>
                ...
                <div class="flex justify-between items-center bg-amber-50 p-3.5 rounded-xl border border-amber-200 text-xs">
                    <span class="font-extrabold text-[#2D1B4E]"><i class="fa-solid fa-truck-fast text-amber-500 mr-1"></i> Fast Delivery</span>
...
<!-- Category Collections (Wix 5441 Style Cards) -->
<div class="grid grid-cols-1 md:grid-cols-3 gap-7 mb-16">
    <a href="{{ route('catalog.index', ['category' => 'childrens-story-books']) }}" class="bg-[#F5F0FF] border-2 border-purple-200/80 hover:border-purple-600 p-8 rounded-[2rem] shadow-sm hover:shadow-xl transition duration-300 group flex flex-col justify-between">
        <div>
        ...
        <span class="text-xs font-extrabold text-purple-700 flex items-center gap-1 group-hover:translate-x-1 transition">Browse Story Books <i class="fa-solid fa-chevron-right text-[10px]"></i></span>
    </a>

    <a href="{{ route('catalog.index', ['category' => 'jigsaw-puzzles']) }}" class="bg-[#FFFBEB] border-2 border-amber-200/80 hover:border-amber-500 p-8 rounded-[2rem] shadow-sm hover:shadow-xl transition duration-300 group flex flex-col justify-between">
        <div>
        ...
        <span class="text-xs font-extrabold text-amber-800 flex items-center gap-1 group-hover:translate-x-1 transition">Browse Puzzles <i class="fa-solid fa-chevron-right text-[10px]"></i></span>
    </a>

    <a href="{{ route('catalog.index', ['category' => 'colouring-books']) }}" class="bg-[#FDF2F8] border-2 border-pink-200/80 hover:border-pink-500 p-8 rounded-[2rem] shadow-sm hover:shadow-xl transition duration-300 group flex flex-col justify-between">
        <div>
        ...
        <span class="text-xs font-extrabold text-pink-700 flex items-center gap-1 group-hover:translate-x-1 transition">Browse Colouring Books <i class="fa-solid fa-chevron-right text-[10px]"></i></span>
    </a>
</div>

<!-- Meet The Author Highlight Section (WrittenbyPD) -->
<div class="bg-gradient-to-r from-purple-900 to-[#2D1B4E] text-white rounded-[2.5rem] p-8 md:p-14 mb-16 shadow-xl border-4 border-[#FBBF24] flex flex-col md:flex-row items-center gap-9">
    <div class="w-36 h-36 md:w-44 md:h-44 bg-[#FBBF24] text-[#2D1B4E] rounded-full flex items-center justify-center font-whimsical font-bold text-5xl shadow-2xl flex-shrink-0 border-4 border-white">
    ...
    <div>
        ...
        <h2 class="font-whimsical font-bold text-3xl md:text-4xl text-white mb-3">WrittenbyPD (800bypd)</h2>
        ...
        <div class="flex flex-wrap items-center gap-5 text-xs font-bold text-amber-300">
            <span><i class="fa-solid fa-location-dot text-amber-400 mr-1"></i> Based in Lagos, Nigeria</span>
            <span><i class="fa-brands fa-spotify text-emerald-400 mr-1"></i> Spotify Audiobook Producer</span>
        </div>
    </div>
</div>

<!-- Bestsellers Grid (Wix Product Display Cards) -->
<div class="mb-16">
    <div class="flex justify-between items-end mb-8">
        <div>
            ...
            <h2 class="font-whimsical font-bold text-3xl text-slate-900 mt-2">Curated Children's Products</h2>
        </div>
        <a href="{{ route('catalog.index') }}" class="text-xs font-extrabold text-purple-700 hover:underline flex items-center gap-1">View Full Catalog <i class="fa-solid fa-chevron-right text-[10px]"></i></a>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        @forelse($featuredProducts as $product)
            <div class="bg-white border border-slate-200/90 rounded-[2.2rem] overflow-hidden shadow-sm hover:shadow-xl transition duration-300 group flex flex-col justify-between">
                <div>
                    <div class="h-64 bg-[#FFFBF5] relative overflow-hidden flex items-center justify-center border-b border-slate-100">
                        ...
                        @else
                            <div class="text-center p-6">
                                <i class="fa-solid fa-book-open-reader text-6xl text-amber-400 mb-2"></i>
                ...
                <div class="p-6 pt-0 flex items-center justify-between mt-auto">
                    ...
                    <form action="{{ route('cart.add', $product->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="bg-[#2D1B4E] hover:bg-purple-900 text-white text-xs font-extrabold px-5 py-2.5 rounded-full flex items-center gap-1.5 shadow-md transition transform hover:-translate-y-0.5">
                            <i class="fa-solid fa-bag-shopping text-amber-400"></i> Add
                        </button>
                    </form>
                </div>
            </div>
        @empty
            <div class="col-span-full py-12 text-center text-slate-500 bg-white border rounded-[2rem]">
                <i class="fa-solid fa-book text-5xl text-amber-400 mb-3"></i>
