    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div>
            <div class="h-72 sm:h-80 lg:h-96 bg-purple-50/50 rounded-2xl overflow-hidden mb-4 relative flex items-center justify-center border border-purple-100">
                ...
                @endif
                <span class="absolute top-4 left-4 bg-emerald-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow flex items-center gap-1">
        ...
        <div class="flex flex-col justify-between">
            <div>
                ...
                <h1 class="font-whimsical text-2xl lg:text-3xl font-bold text-[#2D1B4E] mb-2 leading-tight">{{ $product->name }}</h1>
                ...
                <div class="text-xs text-purple-900 bg-purple-50/80 border border-purple-100 p-3.5 rounded-2xl mb-6 flex items-start gap-2">
                    <i class="bi bi-stars text-amber-500 text-base mt-0.5"></i>
                    <div>
                ...
                <div class="flex items-baseline gap-3 mb-6">
                    ...
                    @if($product->sale_price)
                        ...
                        <span class="bg-amber-400 text-purple-950 text-xs font-black px-2.5 py-0.5 rounded-full">Discount Applied</span>
                    @endif
                </div>
                ...
                <form action="{{ route('cart.add', $product->id) }}" method="POST" class="space-y-4 mb-6">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <input type="number" name="quantity" value="1" min="1" max="{{ $product->stock_quantity }}" class="w-20 border border-slate-300 rounded-xl py-2 px-3 text-center text-sm font-bold focus:ring-2 focus:ring-amber-400 focus:outline-none">
                    </div>

                    <button type="submit" class="w-full bg-[#2D1B4E] hover:bg-purple-900 text-[#FBBF24] font-extrabold py-4 rounded-2xl shadow-lg hover:shadow-xl transition text-sm flex items-center justify-center gap-2">
                        <i class="bi bi-bag-fill text-amber-400"></i> Add to Cart (<span class="price-convertible" data-price-ngn="{{ $product->sale_price ?? $product->price }}">{{ $product->formatted_price }}</span>)
                    </button>
                </form>
            </div>

            <!-- 3-Point Trust Signal Row -->
            <div class="border-t border-slate-100 pt-4 grid grid-cols-3 gap-2 text-center text-xs text-slate-600 bg-amber-50/40 p-3.5 rounded-2xl">
                <div>
                    <i class="bi bi-truck text-purple-600 text-base mb-1 block"></i>
                ...
                <div>
                    <i class="bi bi-shield-check text-emerald-600 text-base mb-1 block"></i>
                    <span class="font-bold block text-slate-800">Paystack & Bank</span>
                    <span class="text-[10px] text-slate-400">100% Secured</span>
                </div>
                <div>
                    <i class="bi bi-spotify text-emerald-500 text-base mb-1 block"></i>
    ...
    <div class="mt-12 border-t border-slate-100 pt-8">
        ...
        <div class="mt-8 bg-amber-50/50 p-6 rounded-3xl border border-amber-200/60">
            <h4 class="font-whimsical font-bold text-slate-900 mb-4 text-base flex items-center gap-2">
                <i class="bi bi-star-fill text-amber-500"></i> Verified Customer Reviews
            </h4>
            <div class="space-y-4">
                @forelse($product->reviews as $review)
                    <div class="bg-white p-4 rounded-2xl border border-slate-200 text-xs shadow-sm">
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="font-bold text-slate-900">{{ $review->customer_name }} <span class="text-emerald-600 text-[10px] font-normal ml-1"><i class="bi bi-check-circle-fill"></i> Verified Buyer</span></span>
